...
app_code = '''"""
AI College Assistant — Streamlit + RAG (Hackathon-Ready)
--------------------------------------------------------
Features:
- Upload multiple PDFs/TXTs of syllabus, notes, and past papers
- RAG chat over your documents with source citations (filename + page)
- Exam Prep mode: generate MCQs/flashcards from retrieved context
- Quick Summaries mode: TL;DR of a unit/topic
- Persist/Reload vector index across runs (optional)

How to run (quick):
1) Python 3.10+ recommended
2) pip install -r requirements.txt  (see inline list below)
3) Set your LLM key (for OpenAI):  export OPENAI_API_KEY=your_key  (or on Windows: set ...)
4) streamlit run app.py

Inline requirements (paste into requirements.txt):
-------------------------------------------------
streamlit>=1.36
langchain>=0.2.7
...
Notes:
- Uses free local embeddings (sentence-transformers) by default for speed & cost.
- Uses OpenAI for the LLM by default (set OPENAI_API_KEY). Swap to other providers if needed.
- Designed to be hackathon-friendly: single file, minimal config, clear UX.
"""
...
if not OPENAI_API_KEY:
    st.warning("OpenAI API key not set. Set OPENAI_API_KEY env var to enable answers.")
        ...
        if not st.session_state.vectorstore:
            ...
            with st.spinner("Retrieving context & creating MCQs…"):
                ...
                try:
                    ...
                    mcqs = output_parser.parse(response)
                except Exception as e:
                    mcqs = f"LLM error: {e}"
    ...
    if sum_btn:
        if not st.session_state.vectorstore:
            ...
            with st.spinner("Retrieving context & summarizing…"):
                ...
                try:
                    response = llm.invoke(msgs)
                    tl_dr = output_parser.parse(response)
                except Exception as e:
                    tl_dr = f"LLM error: {e}"
